...
static int _directvideo = 1;
static int _wscroll = 1;
...
static void __fill_text_info(void)
{
	...
	__text_info.winleft = 0;	//ToDo
	__text_info.wintop = 0;	//ToDo
	__text_info.winright = 0;	//ToDo
	__text_info.winbottom = 0;	//ToDo
	__text_info.currmode = 0;	//ToDo
}
...
void clrscr(void)
{
	...
	{
		FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), __FOREGROUND + (__BACKGROUND << 4), __text_info.screenwidth, (COORD) { __CONIO_LEFT, i }, &written);
		FillConsoleOutputCharacter(GetStdHandle(STD_OUTPUT_HANDLE), ' ', __text_info.screenwidth, (COORD) { __CONIO_LEFT, i}, &written);
	}
	gotoxy(1, 1);
}
...
void clreol(void)
{
	...
	FillConsoleOutputAttribute(GetStdHandle(STD_OUTPUT_HANDLE), __FOREGROUND + (__BACKGROUND << 4), __text_info.screenwidth - __text_info.curx + 1, coord, &written);
...
void delline(void)
{
	...
	fillchar.Attributes = __FOREGROUND + (__BACKGROUND << 4);
		...
		ScrollConsoleScreenBufferW(GetStdHandle(STD_OUTPUT_HANDLE), &rect, NULL, coord, &fillchar);
	#else
		fillchar.Char.AsciiChar = ' ';
		ScrollConsoleScreenBufferA(GetStdHandle(STD_OUTPUT_HANDLE), &rect, NULL, coord, &fillchar);
	#endif
	gotoxy(__text_info.curx, __text_info.cury);
}
...
void gettext(int left, int top, int right, int bottom, struct char_info * buf)
{
	...
	__fill_text_info();
	r = (SMALL_RECT) { __CONIO_LEFT + left - 1, __CONIO_TOP + top - 1, __CONIO_LEFT + right - 1, __CONIO_TOP + bottom - 1 };
	size.X = right - left + 1;
	size.Y = bottom - top + 1;
	buffer = malloc(size.X * size.Y * sizeof(CHAR_INFO));

	ReadConsoleOutput(GetStdHandle(STD_OUTPUT_HANDLE), (PCHAR_INFO)buffer, size, (COORD) { 0, 0 }, &r);
...
void _setcursortype(int type)
{
	...
	{
		Info.bVisible = TRUE;
		if (type == _SOLIDCURSOR)
		{
			Info.dwSize = 100;
		}
		else if (type == _NORMALCURSOR)
		{
			Info.dwSize = 20;
		}
	}
...
void switchbackground(int color)
{
	...
	gettext(1, 1, __text_info.screenwidth, __text_info.screenheight, buffer);
